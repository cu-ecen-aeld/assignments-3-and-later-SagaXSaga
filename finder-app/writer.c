...
int main(int argc , char* argv[]) {
    ...
    file = fopen(argv[1],"w");  // Open the file provided as the first command line argument in write mode and assign the file pointer
    if (file == NULL) {         // Check if the file couldn't be opened
        syslog(LOG_ERR, "Couldn't open file %s", argv[1]);    // Log an error message indicating the failure to open the file
        exit(1);                // Exit the program with an error code of 1
    } else {
